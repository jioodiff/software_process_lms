...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="md:col-span-2">
        <div class="bg-white rounded-2xl shadow-[0_2px_10px_rgb(0,0,0,0.02)] border border-slate-100 p-6 mb-6">
            <h3 class="text-lg font-bold text-slate-800 mb-4 pb-4 border-b border-slate-100">Informasi Pengguna</h3>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Nomor WhatsApp</div>
                    <div class="text-slate-800 font-medium">{{ $user->no_whatsapp ?? '-' }}</div>
                </div>
                <div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl border gap-4 {{ $user->is_active ? 'border-red-100 bg-red-50/50' : 'border-green-100 bg-green-50/50' }}">
                    <div>
                        ...
                        <div class="text-sm mt-0.5 {{ $user->is_active ? 'text-red-600/80' : 'text-green-600/80' }}">{{ $user->is_active ? 'Pengguna tidak akan dapat login atau melakukan peminjaman.' : 'Pengguna akan dapat kembali login dan melakukan peminjaman.' }}</div>
                    </div>
