...
<div class="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
    <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-wrap gap-3 w-full xl:w-auto">
        <div class="relative flex-1 sm:w-96 min-w-[200px]">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                <i data-lucide="search" class="w-4 h-4 text-slate-400"></i>
            </div>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama lengkap atau NIM/NUPTK..." class="block w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-[0_2px_10px_rgb(0,0,0,0.02)] outline-none transition-all">
        </div>
        <select name="role" class="border border-slate-200 rounded-xl text-sm py-2.5 px-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-[0_2px_10px_rgb(0,0,0,0.02)] outline-none transition-all">
            <option value="">Semua Role</option>
            <option value="mahasiswa" {{ request('role') == 'mahasiswa' ? 'selected' : '' }}>Mahasiswa</option>
            <option value="dosen" {{ request('role') == 'dosen' ? 'selected' : '' }}>Dosen</option>
        </select>
        <select name="prodi" class="border border-slate-200 rounded-xl text-sm py-2.5 px-4 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white shadow-[0_2px_10px_rgb(0,0,0,0.02)] outline-none transition-all max-w-[200px] truncate">
        ...
        @if(request()->anyFilled(['search', 'role', 'prodi', 'status']))
                        ...
                        <td class="px-6 py-4 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="text-[11px] text-slate-500 font-mono mt-0.5">
                                    {{ $user->nim }} • {{ $user->email }}
                                    <span class="ml-2 inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium {{ $user->isDosen() ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700' }}">
                                        {{ ucfirst($user->role) }}
                                    </span>
                                </div>
                            </div>
                                        ...
                                        @if($user->is_active && $user->has_active_borrowing)
                                            <div class="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-xl mb-4 text-sm font-medium">
                                                <div class="flex gap-2 items-start">
                                                    ...
                                                    <p class="whitespace-normal break-words leading-relaxed">Warning: Pengguna masih memiliki peminjaman aktif. Pastikan alat dikembalikan sebelum menonaktifkan.</p>
                                                </div>
                                            </div>
                                            <p class="text-slate-500 mb-6 text-sm whitespace-normal">Lanjutkan penonaktifan akun <strong>{{ $user->nama_lengkap }}</strong>?</p>
                                        @else
                                            <p class="text-slate-500 mb-6 text-sm whitespace-normal">Yakin ingin {{ $user->is_active ? 'menonaktifkan' : 'mengaktifkan' }} akun pengguna <strong>{{ $user->nama_lengkap }}</strong>?</p>
                                        @endif
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                            ...
                            <p class="text-base font-semibold text-slate-700 mb-1">Tidak ada data pengguna</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
