                ...
                @forelse($items as $item)
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @if($item->trashed())
                                <span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-500 mt-1 border border-slate-200">Dinonaktifkan</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-1">
                                    ...
                                    <div x-data="{ open: false }" class="inline-block">
                                        ...
                                        <div x-show="open" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" @keydown.window.escape="open = false" style="display: none;">
                                            <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 mx-4 text-left">
                                                ...
                                                <p class="text-slate-500 mb-6 text-sm whitespace-normal">Yakin ingin mengaktifkan kembali inventaris ini?</p>
                                                <div class="flex justify-end gap-3">
                                                    ...
                                                    <form action="{{ route('admin.items.restore', $item->id) }}" method="POST" class="inline-block">
                                                        @csrf
                                ...
                                @else
                                    ...
                                    <div x-data="{ open: false }" class="inline-block">
                                        <button type="button" @click="open = true" class="p-2 text-red-600 hover:bg-red-50 rounded-xl transition-colors tooltip" title="Nonaktifkan">
                                        ...
                                        <div x-show="open" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" @keydown.window.escape="open = false" style="display: none;">
                                            <div class="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 mx-4 text-left">
                                                <div class="flex items-center gap-3 mb-4">
                                                    ...
                                                    <h3 class="text-lg font-bold text-slate-800">Konfirmasi Nonaktifkan</h3>
                                                </div>
                                                <p class="text-slate-500 mb-6 text-sm whitespace-normal">Yakin ingin menonaktifkan inventaris barang <strong>{{ $item->nama_barang }}</strong> ini?</p>
                                                <div class="flex justify-end gap-3">
                                                    <button type="button" @click="open = false" class="px-4 py-2 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-xl text-sm font-semibold transition-colors shadow-sm">Batal</button>
