        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                ...
                <div class="md:col-span-2">
                    <p class="text-xs text-slate-500 mb-1">Nomor WhatsApp</p>
                    <p class="font-medium text-slate-900">{{ $borrowing->mahasiswa->no_whatsapp ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="p-6">
                <div class="flex flex-col md:flex-row gap-6 mb-6 relative">
                    ...
                    <div class="hidden md:flex absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 items-center justify-center z-10">
                        <i data-lucide="arrow-right" class="w-5 h-5 text-indigo-300"></i>
                    </div>
                    ...
                    <div class="flex-1 bg-indigo-50/50 border border-indigo-100 rounded-xl p-4">
                        <div class="flex justify-between items-start">
                            ...
                            @if($borrowing->isOverdue())
                                <div class="flex items-center gap-1.5 bg-rose-100 text-rose-700 px-2.5 py-1 rounded-md text-xs font-bold border border-rose-200 shadow-sm animate-pulse mt-1">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-1.5">
                                        @if($item->kondisi_detail)
                                            @php $kondisiDetail = json_decode($item->kondisi_detail, true); @endphp
                                            <div class="flex flex-wrap gap-1">
                                                @if(is_array($kondisiDetail))
                                                    @foreach($kondisiDetail as $k => $v)
                                                        <span class="inline-flex items-center rounded bg-{{ $k == 'Baik' ? 'emerald' : 'rose' }}-50 px-2 py-0.5 text-[10px] font-medium text-{{ $k == 'Baik' ? 'emerald' : 'rose' }}-700 ring-1 ring-inset ring-{{ $k == 'Baik' ? 'emerald' : 'rose' }}-600/10">
                                                            {{ $v }} {{ $k }}
                                                        </span>
                                                    @endforeach
                                                @endif
                                        ...
                                        @else
                                            @if($item->kondisi_saat_kembali == 'Baik')
                                                <span class="text-emerald-600 flex items-center gap-1 text-xs font-medium"><i data-lucide="check" class="w-3.5 h-3.5"></i> Baik</span>
                                            @else
                                                <span class="text-rose-600 flex items-center gap-1 text-xs font-medium"><i data-lucide="alert-triangle" class="w-3.5 h-3.5"></i> {{ $item->kondisi_saat_kembali }}</span>
                                            @endif
                                        @endif
                                        
                                        @if($item->catatan_pengembalian)
                                            <div class="text-[11px] text-slate-600 bg-slate-50 p-2 rounded-lg border border-slate-100 max-w-xs whitespace-normal leading-relaxed">
                                                <span class="font-semibold text-slate-700 block mb-0.5">Catatan Admin:</span>
    ...
    <div class="space-y-6">
                        ...
                        <div x-data="{ open: false }">
                            ...
                            <div x-show="open" x-cloak class="mt-4 p-4 bg-rose-50 border border-rose-100 rounded-xl" x-transition>
                                <form action="{{ route('admin.borrowings.reject', $borrowing) }}" method="POST">
                                    ...
                                    <textarea name="catatan_admin" required rows="2" placeholder="Alasan penolakan..." class="w-full rounded-xl border border-rose-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-colors bg-white shadow-sm mb-3"></textarea>
        ...
        @elseif($borrowing->status == 'Disetujui')
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <p class="text-sm text-slate-500 mb-6">Peminjam akan mengambil alat. Konfirmasi jika alat sudah diserahkan.</p>
        ...
        @elseif($borrowing->status == 'Dipinjam')
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <p class="text-sm text-slate-500 mb-6">Peminjam mengembalikan alat. Cek kondisi alat sebelum mencatat.</p>
                <a href="{{ route('admin.borrowings.return', $borrowing) }}" class="w-full flex items-center justify-center gap-2 bg-[#155DFC] hover:bg-[#114bca] text-white py-3 px-4 rounded-xl font-semibold shadow-sm transition-all hover:shadow-md">
