...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-800">Mutasi Stok: {{ $tool->nama_alat }}</h2>
            <p class="text-sm text-slate-500">Kode: {{ $tool->kode_alat }} • Stok Fisik Saat Ini: <span class="font-bold text-indigo-600">{{ $tool->stok_total }} unit</span></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
        ...
        <form action="{{ route('admin.tools.storeMutasi', $tool) }}" method="POST" class="p-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    ...
                    <select name="tipe_mutasi" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white shadow-sm">
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" min="1" required class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white shadow-sm">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5.5">Keterangan Tambahan</label>
                    <div class="flex gap-2">
                        <input type="text" name="keterangan" placeholder="Contoh: Alat rusak, Pembelian baru..." class="flex-1 rounded-xl border border-slate-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors bg-white shadow-sm">
