...
class ToolController extends Controller
{
    ...
    public function edit(Tool $tool)
    {
        ...
        return view('admin.tools.edit', compact('tool', 'activeBorrowings'));
    }
    ...
    public function update(Request $request, Tool $tool)
    {
        ...
        if ($request->status_alat !== $tool->status_alat) {
            $activeBorrowings = $tool->borrowingItems()
                ->whereHas('borrowing', fn($q) => $q->whereIn('status', ['Disetujui', 'Dipinjam']))
                ->exists();

            if ($activeBorrowings) {
                return back()->with('error', 'Status alat tidak bisa diubah karena ada unit yang sedang dipinjam atau disetujui.')->withInput();
            }
        }
    ...
    public function restore($id)
    {
        ...
        $before = [
            'status' => 'Dinonaktifkan (Soft Deleted)',
            'deleted_at' => $tool->deleted_at ? $tool->deleted_at->format('Y-m-d H:i:s') : null
        ];
        ...
        AuditLog::record(
            ...
            [
                ...
                'deleted_at' => null,
                'waktu_restore' => now()->format('Y-m-d H:i:s')
            ]
        );
